echo "Длительность событий DBMSSQL в разрезе контекстов"
time grep -r ".*" -H /c/v7/logs/*/*.log  | \
perl -ne '
    use 5;
    ...
    if(/log:\d\d:\d\d\.\d+-\d+,(\w+),/){            #если в строке есть идентификатор начала строки и это наш тип события
        if($1 eq "DBMSSQL"){                           #первоначальный отбор по событиям
...
perl -ne '                                          #perl умеет работать как AWK
    use 5;
    $lines+=1;
    if(m/dur=(\d+),evnt=DBMSSQL.*Context=(.*)/){
        $dur_ttl+=$1/1000;
        $dur{$2}+=$1/1000;
        $cnt_ttl+=1;
        $cnt{$2}+=1;
